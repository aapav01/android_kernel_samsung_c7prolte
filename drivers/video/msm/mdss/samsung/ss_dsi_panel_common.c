...
static void mdss_samsung_event_frame_update(struct mdss_panel_data *pdata, int event, void *arg)
{
	...
	if(vdd->multires_stat.is_support && (vdd->multires_stat.skip_frame > 0)) {
		if(--vdd->multires_stat.skip_frame == 0) {
			LCD_INFO("DISPLAY_ON after sending multires cmd\n");
			mdss_samsung_send_cmd(ctrl, PANEL_DISPLAY_ON);
		}
	}
}
...
void samsung_image_dump(void)
{
	...
	*/
	...
	LCD_ERR("dst_pos : 0x%pK split_mode : %s xres :%d yres :%d\n", dst_fb_pos,
